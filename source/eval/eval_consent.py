...
def extract_scores(conversation_turn):
    ...
    if pos < 0:
        ...
        conversation_turn = conversation_turn[pos + len("\n\nTo be continued...\n\n"):]
        conversation_turn = conversation_turn.strip()
        conversation_turn = conversation_turn[conversation_turn.find("\n\n"):].strip()
        conversation_turn = conversation_turn.strip()
        candidates = conversation_turn.split("\n---")
        for l in range(min(CUT_AT, len(candidates))):
            candidate = candidates[l].strip()
...
print_effectiveness("MRR measure", mrr_evaluator.evaluate(run), "recip_rank", "consent_mrr_" + str(CUT_AT))

if CUT_AT > 1:
    ndcg_evaluator = pytrec_eval.RelevanceEvaluator(query_id_to_judgements, {"ndcg_cut.3"}, relevance_level=1)
    print_effectiveness("nDCG measure", ndcg_evaluator.evaluate(run), "ndcg_cut_3", "consent_ndcg_" + str(CUT_AT))
